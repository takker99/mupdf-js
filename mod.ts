...
// @deno-types=./mupdf-wasm.d.ts
import libmupdf_wasm, { type Pointer } from "./build/release/mupdf-wasm.mjs";
...
function Malloc<T>(size: number) {
  return libmupdf._wasm_malloc(size) as Pointer<T>;
}

function Free<T>(ptr: Pointer<T>) {
...
const _wasm_string: [Pointer<"char">, Pointer<"char">] = [
  0 as Pointer<"char">,
  0 as Pointer<"char">,
];

function checkType(value: unknown, type: unknown) {
...
function checkPoint(value: unknown): asserts value is Point {
  if (!Array.isArray(value) || value.length !== 2) {
    throw new TypeError("expected point");
  }
}

function checkRect(value: unknown): asserts value is Rect {
  if (!Array.isArray(value) || value.length !== 4) {
    throw new TypeError("expected rectangle");
  }
}

function checkMatrix(value: unknown): asserts value is Matrix {
  if (!Array.isArray(value) || value.length !== 6) {
    throw new TypeError("expected matrix");
  }
}

function checkQuad(value: unknown): asserts value is Quad {
  if (!Array.isArray(value) || value.length !== 8) {
    throw new TypeError("expected quad");
  }
}

function checkColor(value: unknown): asserts value is Color {
  if (
    !Array.isArray(value) ||
...
function allocateUTF8(str: string) {
  ...
  const pointer = Malloc<"XXX">(size) as unknown as Pointer<"char">;
  libmupdf.stringToUTF8(str, pointer, size);
  return pointer;
}

function STRING_N(s: string, i: 0 | 1) {
  if (_wasm_string[i]) {
    Free(_wasm_string[i]);
...
export class Buffer extends Userdata<"fz_buffer"> {
  ...
  asUint8Array() {
    const data = libmupdf._wasm_buffer_get_data(this.pointer);
    const size = libmupdf._wasm_buffer_get_len(this.pointer);
...
export class ColorSpace extends Userdata<"fz_colorspace"> {
  ...
  getType() {
    return ColorSpace
      .COLORSPACE_TYPES[libmupdf._wasm_colorspace_get_type(this.pointer)] ||
      "None";
  }
...
export class StrokeState extends Userdata<"fz_stroke_state"> {
  ...
  setLineCap(j: LineCap) {
    const jj = ENUM<LineCap>(j, StrokeState.LINE_CAP);
  ...
  setLineJoin(j: LineJoin) {
    const jj = ENUM<LineJoin>(j, StrokeState.LINE_JOIN);
...
export class StructuredText extends Userdata<"fz_stext_page"> {
  ...
  walk(walker: StructuredTextWalker) {
    ...
    while (block) {
      ...
      const block_bbox = fromRect(libmupdf._wasm_stext_block_get_bbox(block));
      ...
      if (block_type === 1) {
        ...
        while (line) {
          const line_bbox = fromRect(libmupdf._wasm_stext_line_get_bbox(line));
          const line_wmode = libmupdf._wasm_stext_line_get_wmode(line);
          const line_dir = fromPoint(libmupdf._wasm_stext_line_get_dir(line));
          ...
          if (walker.onChar) {
            ...
            while (ch) {
              const ch_rune = String.fromCharCode(
                libmupdf._wasm_stext_char_get_c(ch),
              );
              const ch_origin = fromPoint(
                libmupdf._wasm_stext_char_get_origin(ch),
              );
              const ch_font = new Font(libmupdf._wasm_stext_char_get_font(ch));
              const ch_size = libmupdf._wasm_stext_char_get_size(ch);
              const ch_quad = fromQuad(libmupdf._wasm_stext_char_get_quad(ch));
...
export class Document extends Userdata<"any_document"> {
  ...
  hasPermission(perm: DocumentPermission) {
    const perm_ix = Document.PERMISSION[perm];
    return !!libmupdf._wasm_has_permission(this.pointer, perm_ix);
  }

  getMetaData(key: string) {
    const value = libmupdf._wasm_lookup_metadata(this.pointer, STRING(key));
    if (value) {
  ...
  loadPage(index: number) {
    const fz_ptr = libmupdf._wasm_load_page(this.pointer, index);
    if (this instanceof PDFDocument) {
      const pdf_ptr = libmupdf._wasm_pdf_page_from_fz_page(fz_ptr);
      if (pdf_ptr) {
  ...
  loadOutline() {
    ...
    function to_outline(outline: Pointer<"fz_outline">) {
      ...
      while (outline) {
        ...
        const page = libmupdf._wasm_outline_get_page(doc, outline);
        if (page >= 0) {
          item.page = page;
        }
    ...
    const root = libmupdf._wasm_load_outline(doc);
    if (root) {
  ...
  resolveLinkDestination(link: string | Link): LinkDest {
    ...
    return {
      type: Document
        .LINK_DEST[libmupdf._wasm_link_dest_get_type(dest)] as LinkDestType,
...
export class OutlineIterator extends Userdata<"fz_outline_iterator"> {
  ...
  item() {
    const item = libmupdf._wasm_outline_iterator_item(this.pointer);
    if (item) {
      const title_ptr = libmupdf._wasm_outline_item_get_title(item);
      const uri_ptr = libmupdf._wasm_outline_item_get_uri(item);
      const is_open = libmupdf._wasm_outline_item_get_is_open(item);
      return {
...
export class Page extends Userdata<"any_page"> {
  ...
  getBounds(box: PageBox = "CropBox") {
    const box_ix = ENUM<PageBox>(box, Page.BOXES);
...
export class PDFDocument extends Document {
  ...
  constructor(
    ...
    if (typeof arg1 === "undefined") {
      ...
      const doc = Document.openDocument(arg1, "application/pdf");
  ...
  _toPDFObject(obj: unknown) {
    ...
    if (obj instanceof Array) {
      const result = this.newArray(obj.length);
    ...
    if (obj instanceof Object) {
      const result = this.newDictionary();
  ...
  createObject() {
    const num = libmupdf._wasm_pdf_create_object(this.pointer);
  ...
  addStream(buf: AnyBuffer, obj: unknown) {
    return this._fromPDFObjectNew(
  ...
  addRawStream(buf: AnyBuffer, obj: unknown) {
    return this._fromPDFObjectNew(
      libmupdf._wasm_pdf_add_stream(
        this.pointer,
        BUFFER(buf),
  ...
  addSimpleFont(font: Font, encoding: FontSimpleEncoding = "Latin") {
    checkType(font, Font);
    const encoding_ix = ENUM<FontSimpleEncoding>(
      encoding,
  ...
  _rewriteEmbeddedFiles(efs: Record<string, PDFObject>) {
    ...
    if (!root_names.isDictionary()) {
      root_names = root.put("Names", this.newDictionary(1)) as PDFObject;
    }
    const root_names_efs = root_names.put(
      "EmbeddedFiles",
      this.newDictionary(1),
    );
    const root_names_efs_names = root_names_efs.put(
      "Names",
      this.newArray(efs_keys.length * 2),
    );
  ...
  setJSEventListener(_listener: unknown) {
    throw "TODO";
  }
...
export class PDFObject extends Userdata<"pdf_obj"> {
  ...
  static readonly Null = new PDFObject(
  ...
  put(key: number | string | PDFObject, value: unknown) {
    value = this._doc._toPDFObject(value);
    if (typeof key === "number") {
      libmupdf._wasm_pdf_array_put(
        this.pointer,
        key,
        (value as PDFObject).pointer,
      );
    } else if (key instanceof PDFObject) {
      libmupdf._wasm_pdf_dict_put(
        this.pointer,
        key.pointer,
        (value as PDFObject).pointer,
      );
  ...
  push(value: unknown) {
    value = this._doc._toPDFObject(value);
    libmupdf._wasm_pdf_array_push(this.pointer, (value as PDFObject).pointer);
    return value;
  }
  ...
  forEach(fn: (val: PDFObject, key: number | string, self: PDFObject) => void) {
    if (this.isArray()) {
      ...
      for (let i = 0; i < n; ++i) {
        const key = this._doc._fromPDFObjectKeep(
          libmupdf._wasm_pdf_dict_get_key(this.pointer, i),
        );
        const val = this._doc._fromPDFObjectKeep(
  ...
  asJS(seen?: Record<number, PDFObject>): unknown {
    if (this.isIndirect()) {
      ...
      return seen[ref] = this.resolve().asJS(seen) as PDFObject;
    }
...
const $libmupdf_stm_table: Map<number, StreamHandle> = new Map();
...
globalThis.$libmupdf_stm_seek = function (
  id: number,
  pos: number,
  offset: number,
  whence: number,
...
globalThis.$libmupdf_stm_read = function (
  id: number,
  pos: number,
  addr: number,
  size: number,
